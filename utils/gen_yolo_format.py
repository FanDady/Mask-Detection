...
'''
@说明：
此部分代码主要是生成Python版YoloV*格式的数据集模板，不适用于C语言版的Yolo数据集生成
@使用说明：
1、修改根目录路径FILE_ROOT
2、根据代码提示修改对应路径
3、根据需要修改box[0]的标签
...
# 原始数据集
IMAGE_SET_ROOT = FILE_ROOT.joinpath(r"MaskVOC\ImageSets\Main")  # 训练集和验证集的txt文件夹的输入路径
IMAGE_PATH = FILE_ROOT.joinpath(r"MaskVOC\JPEGImages")  # 所有图片的输入路径
ANNOTATIONS_PATH = FILE_ROOT.joinpath(r"MaskVOC\Annotations")  # 数据集标签xml文件的位置
LABELS_ROOT = FILE_ROOT.joinpath(r"MaskVOC\Labels")  # 进行归一化之后的标签保存位置

# YOLO 需要的数据集形式的新数据集
DEST_IMAGES_PATH = Path(r"MaskYOLO\images")  # 区分训练集、测试集、验证集的图片目标路径
DEST_LABELS_PATH = Path(r"MaskYOLO\labels")  # 区分训练集、测试集、验证集的标签文件目标路径
...
def save_label_file(img_jpg_file_name, size, img_box):
    ...
    with open(save_file_name, "a+") as f:
        for box in img_box:
            if box[0] == 'mask':  # 数据集 xml 中的 person 指的是头
                cls_num = 0
            elif box[0] == 'nomask':
                cls_num = 1
            else:
